...
function ensure_document_management_mysql_runned() {
    $mysql_runned = get_option('document_management_mysql_runned');
    if (empty($mysql_runned)) {
        $CI = &get_instance();
        require_once(module_dir_path('document_management', 'install.php'));
        $CI->db->where('name', 'document_management_mysql_runned');
