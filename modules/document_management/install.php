...
if (!$CI->db->table_exists(db_prefix() . 'dmg_items')) {
  $CI->db->query('CREATE TABLE IF NOT EXISTS `' . db_prefix() . "dmg_items` (
  ...
  if (!$CI->db->table_exists(db_prefix() . 'dmg_file_versions')) {
    $CI->db->query('CREATE TABLE IF NOT EXISTS `' . db_prefix() . "dmg_file_versions` (
      `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
      `name` varchar(300) NOT NULL,
  ...
  if (!$CI->db->table_exists(db_prefix() . 'dmg_custom_fields')) {
    $CI->db->query('CREATE TABLE IF NOT EXISTS `' . db_prefix() .'dmg_custom_fields` (
  ...
  if (!$CI->db->table_exists(db_prefix() . 'dmg_audit_logs')) {
    $CI->db->query('CREATE TABLE IF NOT EXISTS `' . db_prefix() . "dmg_audit_logs` (
    `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
    `date` datetime NULL,
    `user_id` int(11) NULL,
  ...
  if (!$CI->db->table_exists(db_prefix() . 'dmg_remiders')) {
    $CI->db->query('CREATE TABLE IF NOT EXISTS `' . db_prefix() . "dmg_remiders` (
    `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
    `file_id` int(11) NULL,
  ...
  if (!$CI->db->table_exists(db_prefix() . 'dmg_share_logs')) {
    $CI->db->query('CREATE TABLE IF NOT EXISTS `' . db_prefix() . "dmg_share_logs` (
      `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
      `item_id` int(11) NULL,
  ...
  if (!$CI->db->table_exists(db_prefix() . 'dmg_approval_setting')) {
    $CI->db->query('CREATE TABLE IF NOT EXISTS `' . db_prefix() .'dmg_approval_setting` (
  ...
  if (!$CI->db->table_exists(db_prefix() . 'dmg_approval_details')) {
    $CI->db->query('CREATE TABLE IF NOT EXISTS `' . db_prefix() .'dmg_approval_details` (
    `id` INT(11) NOT NULL AUTO_INCREMENT,
  ...
  if (!$CI->db->table_exists(db_prefix() . 'dmg_approval_detail_eids')) {
    $CI->db->query('CREATE TABLE IF NOT EXISTS `' . db_prefix() .'dmg_approval_detail_eids` (
    `id` INT(11) NOT NULL AUTO_INCREMENT,
    `rel_id` INT(11) NOT NULL,
    `rel_type` VARCHAR(45) NOT NULL,
    `staffid` VARCHAR(45) NULL,
